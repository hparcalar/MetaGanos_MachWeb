...
export function registerRouterNavigationGuards({ router, api }: VueroAppContext) {
  router.beforeEach(async (to, from) => {
    ...
    if (to.path.match('/auth.*') && userSession.isLoggedIn) {
      ...
      try {
        ...
        if (userSession.user) {
          notyf.success(`Hoşgeldiniz, ${userSession.user.name}`)
          return {
            name: 'app',
          }
        }
        ...
        notyf.error('Oturumunuz hatalı. Yeniden giriş yapmalısınız.')
      ...
      notyf.error({
        message: 'Sisteme tekrar giriş yapmalısınız.',
        duration: 7000,
      })
