...
const filteredData = computed(() => {
  if (!filters.value) {
    ...
    return cards.value.filter((item) => {
      return (
        item.itemGroupCode?.match(filterRe) ||
...
onMounted(async () => {
  try {
    cards.value = (await api.get('ItemGroup')).data
  } catch (error) { }
})
...
<template>
  <div>
    <div class="list-flex-toolbar is-reversed">
      ...
      <VButton v-if="hasAuth('ItemGroups', 'Write')" :color="'info'" :raised="true" icon="feather:plus"
        @click="openDetail(0)">{{ getExpression('NewGroup') }}
      </VButton>
    </div>
          ...
          <template #body>
            <TransitionGroup name="list" tag="div" class="flex-list-inner">
              ...
              <div v-for="item in filteredData" :key="item.id" class="flex-table-item">
                <VFlexTableCell>
                  <span class="">{{ item.itemGroupCode }}</span>
                </VFlexTableCell>
                <VFlexTableCell>
